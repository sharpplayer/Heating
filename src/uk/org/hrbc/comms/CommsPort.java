...
public class CommsPort implements SerialPortEventListener {
  ...
  // '0', '0', 0x05 };
  // private final static byte[] INIT2 = new byte[] { (byte) 0xFE, (byte) 0x01,
    ...
    if (trueData == END_BUFFER) {
      if ((bufferIndex > 1) && (receiveBuffer[bufferIndex - 2] == ESCAPE_CODE)) {
        ...
        if (receiveBuffer[0] == IGNORE_READ) {
          ...
          String key = Character.toString((char) receiveBuffer[3]) + Character.toString((char) receiveBuffer[4])
              + Character.toString((char) receiveBuffer[5]);
          String data = "";
          for (int loop = 9; (receiveBuffer[loop] != END_BUFFER) && (receiveBuffer[loop] != 0x0D); loop++)
            data += (char) (receiveBuffer[loop]);
          // For some reason alarm connection receives junk sometimes at the
          // start
          int ind = data.indexOf(ALARM_MESSAGE);
          if (ind != -1) {
            ind += 8;
            key = Character.toString((char) receiveBuffer[ind - 5]) + Character.toString((char) receiveBuffer[ind - 4])
            ...
            try {
              ...
              debug.add(new DebugItem(DebugItem.INFO, "Failed to Ack ALARM CONNECTION:" + ex.getMessage()));
            }
          } else {
             inData.put(key, data);
          }
